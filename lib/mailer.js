...
class Mailor {
  ...
  static buildMailer(baseDir, options, data) {
    const mailer = Mailor.getMailer(options);
    const extension = `.${options.extname || 'html'}`;

    mailer.options.defaultData = {
      ...mailer.options.defaultData,
      ...data,
    };
    ...
    return fs.readdirSync(baseDir).filter(x => x.includes(extension)).reduce((prev, cur) => {
      const tplName = cur.replace(extension, '').replace(/-([a-z])/g, (_, $1) => $1.toUpperCase());
  ...
  async sendMail({
    template, data, email, subject, ...otherOptions
  } = {}) {
    /* istanbul ignore else */
    ...
    Object.keys(otherOptions).forEach(key => {
      if (otherOptions[key]) mailOptions[key] = otherOptions[key];
    });
